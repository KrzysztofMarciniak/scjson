 ...
 * the map, its old value is updated with the new one. If the key does not exist,
 * a new bucket node is dynamically allocated, and the key is duplicated via
 * `strdup`.
 *
...
void _scj_map_set(scjson self, const char* key, scjson value) {
    ...
    if (self->value.object.map.capacity == 0) {
        ...
        if (!self->value.object.map.buckets) {
            ...
            self->error.message = "scj_map_set.c: bucket alloc failed";
            return;
        }
    }
