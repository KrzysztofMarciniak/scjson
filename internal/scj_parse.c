...
static scjson parse_value(scj_lexer* l) {
        ...
        switch (t.type) {
                case SCJ_T_STRING:
                        ...
                        if (!j->value.string)
                                set_error(j, SCJ_ERR_ALLOC,
                                          "Failed to allocate string", l);
                        return j;
                ...
                default:
                        set_error(j, SCJ_ERR_UNEXPECTED_TOKEN,
                                  "Unexpected token", l);
                        return j;
        }
}
...
static scjson parse_array(scj_lexer* l) {
        ...
        while (1) {
                ...
                if (!val) {
                        set_error(arr, SCJ_ERR_PARSE,
                                  "Failed to parse array value", l);
                        return arr;
                }
                ...
                set_error(arr, SCJ_ERR_UNEXPECTED_TOKEN, "Expected ',' or ']'",
                          l);
                return arr;
        }
...
static scjson parse_object(scj_lexer* l) {
        ...
        while (1) {
                ...
                if (t.type != SCJ_T_STRING) {
                        set_error(obj, SCJ_ERR_INVALID_KEY,
                                  "Expected string key", l);
                        return obj;
                }

                char* key = strndup(t.start, t.len);
                if (!key) {
                        set_error(obj, SCJ_ERR_ALLOC, "Failed to allocate key",
                                  l);
                        return obj;
                }
                ...
                if (t.type != SCJ_T_COLON) {
                        free(key);
                        set_error(obj, SCJ_ERR_UNEXPECTED_TOKEN,
                                  "Expected ':' after key", l);
                        return obj;
                }
                ...
                if (!val) {
                        free(key);
                        set_error(obj, SCJ_ERR_PARSE,
                                  "Failed to parse value for key", l);
                        return obj;
                }
                ...
                set_error(obj, SCJ_ERR_UNEXPECTED_TOKEN, "Expected ',' or '}'",
                          l);
                return obj;
        }
...
scjson _scj_parse(const char* text) {
        ...
        if (!text) {
                set_error(root, SCJ_ERR_INVALID_VALUE, "Input text is NULL",
                          NULL);
                return root;
        }
        ...
        if (!l) {
                set_error(root, SCJ_ERR_ALLOC, "Failed to allocate lexer",
                          NULL);
                return root;
        }
